...
def _create_connection(quick_put, args, kwargs):
    conn = Connection(*args, **kwargs)
    quick_put(conn)
...
class ConnectionPool:
    ...
    _POOL_START_SIZE = 10
    _MAX_ADD_CONACTION_IN_RUNTIME = 10

    def __init__(self, size=10, name=None, *args, **kwargs):
        self._pool = queue.Queue(self._HARD_LIMIT)
        """calculate the pool size need to subtract the first _POOL_START_SIZE connections """
        self._size = size if 0 < size < self._HARD_LIMIT else self._HARD_LIMIT
        """init a counter that we will not pass the user/ heard limit  size """
        self._connection_count = self._size
        self.name = name if name else '-'.join(
            [kwargs.get('host', 'localhost'), str(kwargs.get('port', 3306)),
             kwargs.get('user', ''), kwargs.get('database', '')])

        """cash user parameters"""
        self.args, self.kwargs = args, kwargs

        threads_pool: [] = []
        for _ in range(self._size):
            threads_pool.append(threading.Thread(target=_create_connection, args=(self._pool.put, args, kwargs)))
